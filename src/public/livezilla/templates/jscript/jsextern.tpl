...
function lz_chat_init_search_kb(_reset,_backgroundSearch)
{
    ...
    if(phrase != lz_chat_data.KBLastSearchPhrase)
    {
        if(!lz_chat_data.KBSearchActive)
        {
            ...
            lz_chat_get_frame_object('lz_chat_kb_results').innerHTML = '<br><br><br><img src=\"<!--server-->images/chat_loading.gif\">';
        }
        else
...
{
    var src = "./knowledgebase.php?id=" + _id + "&epc=" + lz_global_base64_url_encode(lz_chat_data.PrimaryColor) + "&etc=" + lz_global_base64_url_encode(lz_chat_data.SecondaryColor);
